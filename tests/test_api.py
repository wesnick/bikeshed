...
@pytest.mark.asyncio
async def test_session_component(client: AsyncClient):
    """Test that the session component returns a 200 status code"""
    response = await client.get("/session", headers={"HX-Request": "true"})
    assert response.status_code == 200
